...
const  compensationboxService  = require("../services/compensationboxService");

const getAll = async (req, res) => {
    try {
        const response = await compensationboxService.getAll();
...
const getId = async (req, res) => {
    try {
        ...
        const response = await compensationboxService.getId(_id);
        if(response.length > 0){
            return messagePersonalized(res,200,'Registros encontrados',response);
        } else {
...
const create = async (req, res) => {
    try {
        const { name } = req.body;
        const response = await compensationboxService.create(name);
...
const update = async (req, res) => {
    try {
        const { _id } = req.params;
        const response = await compensationboxService.update(_id,req.body);
        if(response === null){
...
const deleteId = async (req, res) => {
    try {
        const { _id } = req.params;
        const response = await compensationboxService.deleteId(_id);
        if(response === null) {
