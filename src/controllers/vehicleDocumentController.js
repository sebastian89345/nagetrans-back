...
const getAll = async (req, res) => {
    try {
        const response = await vehicleDocumentService.getAll();
...
const getId = async (req, res) => {
    try {
        ...
        const response = await vehicleDocumentService.getId(_id);
        if(response.length > 0){
            return messagePersonalized(res,200,'Registros encontrados',response);
        } else {
...
const create = async (req, res) => {
    try {
        const body = req.body;
        const response = await vehicleDocumentService.create(body);
...
const update = async (req, res) => {
    try {
        const { _id } = req.params;
        const response = await vehicleDocumentService.update(_id,req.body);
        if(response === null){
...
const deleteId = async (req, res) => {
    try {
        const { _id } = req.params;
        const response = await vehicleDocumentService.deleteId(_id);
        if(response === null) {
