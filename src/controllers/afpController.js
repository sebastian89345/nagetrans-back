...
const  afpService  = require("../services/afpService");

const getAll = async (req, res) => {
    try {
        const response = await afpService.getAll();
...
const getId = async (req, res) => {
    try {
        ...
        const response = await afpService.getId(_id);
        if(response.length > 0){
            return messagePersonalized(res,200,'Registros encontrados',response);
        } else {
...
const create = async (req, res) => {
    try {
        const { name } = req.body;
        const response = await afpService.create(name);
...
const update = async (req, res) => {
    try {
        const { _id } = req.params;
        const response = await afpService.update(_id,req.body);
        if(response === null){
...
const deleteId = async (req, res) => {
    try {
        const { _id } = req.params;
        const response = await afpService.deleteId(_id);
        if(response === null) {
